            ...
            _G.autoPathLoop = spawn(function()
                ...
                while _G.autoPathEnabled and wait(_G.autoPathDelay) do -- Chuyển đổi theo thời gian đã cài đặt
                    ...
                    if isPlayerInMap() then
                        ...
                        if success then
                            if faseValue >= 0 and faseValue <= 1 then
                                maxPath = 1 -- Chỉ đi đường 1
                                print("Fase " .. faseValue .. ": Giới hạn đường đi 1")
...
UnitsUpdateSection:AddToggle("AutoUpdateRandomToggle", {
    ...
    Callback = function(Value)
        ...
        if Value then
            ...
            -- Tạo vòng lặp mới
            spawn(function()
                while autoUpdateRandomEnabled and wait(1) do -- Cập nhật mỗi 3 giây
                    -- Kiểm tra xem có trong map không
